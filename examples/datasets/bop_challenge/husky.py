...
target_bop_objs = bproc.loader.load_bop_objs(bop_dataset_path = os.path.join(args.bop_parent_path, 'Husky'), mm2m = True)
...
bproc.loader.load_bop_intrinsics(bop_dataset_path = os.path.join(args.bop_parent_path, 'Husky'))

# set shading and hide objects
# for obj in (target_bop_objs + lm_dist_bop_objs + tless_dist_bop_objs + ycbv_dist_bop_objs + tyol_dist_bop_objs):
for obj in (target_bop_objs):
...
# load texture for husky
texture = Path("/home/user/Desktop/New Folder/Husky/models/baked_mesh_a6af74a5_tex0.png")
...
for i in range(args.num_scenes):
    ...
    for obj in (sampled_target_bop_objs):
        ...
        elif obj.get_cp("bop_dataset_name") in ["Husky"]:
    ...
    # bproc.object.sample_poses(objects_to_sample = sampled_target_bop_objs + sampled_distractor_bop_objs,sample_pose_func = sample_pose_func, max_tries = 1000)
